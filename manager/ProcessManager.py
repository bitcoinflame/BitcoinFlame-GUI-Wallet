...
## Copyright (c) 2017, The Sumokoin Project (www.sumokoin.org)
'''
Process managers for BitcoinFlamed, BitcoinFlame-wallet-cli and BitcoinFlame-wallet-rpc
'''
...
class BitcoinFlamedManager(ProcessManager):
    def __init__(self, resources_path, log_level=0, block_sync_size=10):
        proc_args = u'%s/bin/BitcoinFlamed --log-level %d --block-sync-size %d --rpc-bind-port %d' % (resources_path, log_level, block_sync_size, RPC_DAEMON_PORT)
        ProcessManager.__init__(self, proc_args, "BitcoinFlamed")
        self.synced = Event()
        self.stopped = Event()
...
class WalletCliManager(ProcessManager):
    ...
    def __init__(self, resources_path, wallet_file_path, wallet_log_path, restore_wallet=False):
        if not restore_wallet:
            wallet_args = u'%s/bin/BitcoinFlame-wallet-cli --generate-new-wallet=%s --log-file=%s' \
                                                % (resources_path, wallet_file_path, wallet_log_path)
        else:
            wallet_args = u'%s/bin/BitcoinFlame-wallet-cli --log-file=%s --restore-deterministic-wallet' \
                                                % (resources_path, wallet_log_path)
        ProcessManager.__init__(self, wallet_args, "BitcoinFlame-wallet-cli")
        self.ready = Event()
        self.last_error = ""
